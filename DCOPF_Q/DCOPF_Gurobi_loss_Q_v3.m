...
E_P_est_old=zeros(nb,1);    %Active Power Fictinous Nodal Demand of iteration-1   (used for iteration damping)

E_Q_est=zeros(nb,1);        %Reactive Power Fictinous Nodal Demand
E_Q_est_old=zeros(nb,1);    %Reactive Power Fictinous Nodal Demand of iteration-1 (used for iteration damping)
...
charging_injection=ones(1,nb)*diag(BD).*baseMVA;            %NOT SURE IF RIGHT : 
...
q_demand=ones(1,nb)*(busdata(:,4).*DF_Q_est)-Qloss_est-charging_injection;
...
qd=busdata(:,4)-diag(BD).*baseMVA;                  %NOT SURE IF RIGHT:
                                                    %Nodal reactive power
                                                    %demand - charging
                                                    %injection of shunt
                                                    %elements at each bus
                                                    
b1=prat(1:length(branchdata(:,1)))+GSF_PP*(pd+E_P_est)+GSF_PQ*(qd+E_Q_est);     %right side of line transfer limit Equation
b2=prat(1:length(branchdata(:,1)))-GSF_PP*(pd+E_P_est)-GSF_PQ*(qd+E_Q_est);     %right side of line transfer limit Equation
...
%% I'm diving the left and right side by the baseMVA to have the Power Injection/Withdrawal in p.u.
...
model.sense = [repmat('<',size(A_P,1),1); repmat('<',size(A_Voltage_P,1),1); repmat('>',size(A_Voltage_P,1),1); repmat('=',size(Aeq_P,1),1); repmat('=',size(Aeq_Q,1),1)];
%% Right side of A*x=b
...
w=0.87;
if iter>1
pn_old=Ag*gendispatch(1:ng,iter-1)-pd-E_P_est_old;
pn=w*pn_old+(1-w)*pn;

qn_old=Ag*gendispatch(ng+1:end,iter-1)-qd-E_Q_est_old;
qn=w*qn_old+(1-w)*qn;
end
